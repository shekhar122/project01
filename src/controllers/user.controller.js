...
const getUserChannelProfile = asyncHandler(async(req, res) => {
   ...
   //aggregation opration
   //TODO: do the console log of this channel value
   const channel = await User.aggregate([
      {//first pipeline is match field pipeline
         $match: {
            username: username?.toLowerCase()
         }
      },
      //now we have one document, will do the lookup now
      {
         $lookup: { //for subscriber
            //in model everything will be converted to lower case and plural
            from: "subscriptions",
            localField: "_id",
   ...
   if(!channel?.length) {
      throw new ApiError(400, "channle doesn't exists")
   }
